...
var nowTimeForLog = {}; //datejs object

// classify message
var operationMode = 'GENERAL_MODE'; //default
var meetingOpMode = 'OFF_MEETING';  //default
var translationMode = 'TRANSLATION_OFF'; //default
...
var captureStart = function () {
    ...
    intervalId = setInterval(function(){
        if(timelimitSec > 0){
            $('#js-recording-button').html('Recognizing:'+ timelimitSec);
    ...
    $('#js-recording-button').html('Recognizing');
...
var captureStop = function() {
    ...
    $('#js-recording-button').html('Start recognizing');
    ...
    function(){
        ...
        $("#js-sending-button").bind("click",
            function(){
                ...
                executeAfterSendMessage(totalResultStrTemp);
            }
        );
        // send chat button
        $("#js-sending-button").keypress(function(e){
            var key = e.which;
            if(key === 13){
              //送信.
              totalResultStrTemp = $('#js-text-textview-temp').val();
...
function typeResultTextTemp(buff) {
    ...
    $('#js-text-textview-temp').val(S(totalResultStrTemp).trim().s);
    $('#js-text-textview-temp')[0].focus();
}
/**
 * history division にbeforeで、elementを書き出す
 */
function typeResultTextHistList(sendText) {
    var datetimeseq = nowTimeForLog.toString('HHmmss-yyMMdd');
    var param = {
      "datetimeseq" : datetimeseq,
      "message" : sendText
    };
    var template = '<li id="{{datetimeseq}}"><label class="checkbox"><input type="checkbox" value="{{datetimeseq}}" id="{{datetimeseq}}"><select><option value="general" selected>--</option><option value="TODO">TODO</option><option value="FIXME">FIXME</option><option value="XXX">XXX</option></select></input><span>  id : {{datetimeseq}}</span><div class="line-sentence"><label for="{{datetimeseq}}">{{message}}</label></div></label></li>';
    var elem = Mustache.to_html(template,param);
    $('#js-ul-history').prepend(elem);
    $('#js-ul-history').sortable({
      revert : true,
      placeholder : 'highlight',
      update : function(e,ui){
        $('#result').text($('#js-ul-history').sortable('toArray'));
      }
    })
    $('#js-text-textview-temp')[0].focus();
}
...
function classifyMessageSuccessCallback(data){
  console.log('----function:classifyMessageSuccessCallback()--');
}
function classifyMessageErrorCallback(data){
  console.log('----function:classifyMessageErrorCallback()--');
}
 ...
 */
function executeAfterSendMessage(textTemp){
  if(textTemp){
    nowTimeForLog = Date.now();
    typeResultTextHistList(totalResultStrTemp);
    totalResultStrTemp = '-----------------------\n<sent>' + nowTimeForLog.toString(' on yyyy/MM/dd(ddd)HH:mm:ss') +'\n' + totalResultStrTemp +  '\n';
...
function clearTempArea(textTemp){
  if(textTemp){
    nowTimeForLog = Date.now();
    totalResultStrTemp = '-----------------------\n<not-sent>' + nowTimeForLog.toString(' on yyyy/MM/dd(ddd)HH:mm:ss') +'\n' + totalResultStrTemp +  '\n';
    totalResultStrLog = totalResultStrLog + totalResultStrTemp;
    totalResultStrTemp = '' ; // clear
    $('#js-text-textview-temp').val('');
    typeResultTextTemp('');
